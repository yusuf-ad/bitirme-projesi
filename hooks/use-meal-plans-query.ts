...
export function useMealPlansQuery(
  ...
  const fetchMealsForDate = useCallback(async (): Promise<FetchMealsResult> => {
    ...
    const { data: items, error: itemsError } = await supabase
      ...
      .select(
        "id, meal_plan_id, spoonacular_recipe_id, recipe_name, recipe_image_url, calories_per_serving, carbs_per_serving, protein_per_serving, fat_per_serving, ready_in_minutes, meal_date, meal_type, is_ai_generated"
      )
